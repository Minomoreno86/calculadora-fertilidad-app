...
export const useSimpleValidation = (
  ...
  const ageValidation = useMemo((): SimpleValidationResult => {
    ...
    if (formValues.age >= 40) {
      return {
        isValid: true,
        message: `Edad ${formValues.age} años - Fertilidad reducida. Evaluación urgente recomendada`,
        type: 'warning'
      };
    }
    ...
    if (formValues.age >= 35) {
      return {
        isValid: true,
        message: `Edad ${formValues.age} años - Declive de fertilidad. Considerar evaluación prioritaria`,
        type: 'warning'
      };
    }
    ...
    return {
      isValid: true,
      message: `Edad ${formValues.age} años - Rango óptimo para fertilidad natural y TRA`,
      type: 'success'
    };
  }, [formValues?.age]);
  ...
  const cycleValidation = useMemo((): SimpleValidationResult | undefined => {
    ...
    if (cycleLength >= 21 && cycleLength <= 35) {
      return {
        isValid: true,
        message: `Ciclo de ${cycleLength} días - Duración normal`,
        type: 'success'
      };
    } else if (cycleLength >= 36 && cycleLength <= 45) {
      return {
        isValid: true,
        message: `Ciclo de ${cycleLength} días - Ciclo largo, posible anovulación`,
        type: 'warning'
      };
    } else if (cycleLength >= 15 && cycleLength <= 20) {
      return {
        isValid: true,
        message: `Ciclo de ${cycleLength} días - Ciclo corto, posible fase lútea insuficiente`,
        type: 'warning'
      };
    } else if (cycleLength > 45) {
      return {
        isValid: true,
        message: `Ciclo de ${cycleLength} días - Oligomenorrea, probable anovulación`,
        type: 'warning'
      };
    } else {
      return {
        isValid: true,
        message: `Ciclo de ${cycleLength} días - Duración muy anormal, requiere evaluación`,
        type: 'warning'
      };
    }
  ...
  const amhValidation = useMemo((): SimpleValidationResult | undefined => {
    if (!formValues?.amhValue || formValues.amhValue <= 0) {
      return undefined; // No mostrar si no hay datos
    }

    const age = formValues.age || 30;
    const amh = formValues.amhValue;
    ...
    if (age < 30) {
      if (amh < 1.2) {
        return {
          isValid: true,
          message: `AMH ${amh.toFixed(1)} ng/ml - Reserva baja para edad <30. Evaluación urgente`,
          type: 'warning'
        };
      } else if (amh > 6.0) {
        return {
          isValid: true,
          message: `AMH ${amh.toFixed(1)} ng/ml - Reserva muy alta. Riesgo SHO en estimulación`,
          type: 'warning'
        };
      }
    } else if (age < 35) {
      if (amh < 1.0) {
        return {
          isValid: true,
          message: `AMH ${amh.toFixed(1)} ng/ml - Reserva baja para edad 30-35. TRA prioritario`,
          type: 'warning'
        };
      } else if (amh > 5.0) {
        return {
          isValid: true,
          message: `AMH ${amh.toFixed(1)} ng/ml - Reserva alta. Protocolo suave recomendado`,
          type: 'warning'
        };
      }
    } else if (age >= 35) {
      if (amh < 0.8) {
        return {
          isValid: true,
          message: `AMH ${amh.toFixed(1)} ng/ml - Reserva muy baja para edad ≥35. Consejería urgente`,
          type: 'warning'
        };
      }
    }
    ...
    return {
      isValid: true,
      message: `AMH ${amh.toFixed(1)} ng/ml - Reserva adecuada para edad ${age} años`,
      type: 'success'
    };
