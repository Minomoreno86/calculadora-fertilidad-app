// src/core/domain/services/treatmentSuggesterPremium.ts
import { EvaluationState, TreatmentSuggestion, UserInput, Factors, HsgResult, PolypType, AdenomyosisType } from '../models';
import { clinicalContentLibraryPremium } from '../logic/clinicalContentLibraryPremium'; // Importar la biblioteca PREMIUM

/**
 * Helper para obtener una sugerencia de tratamiento del clinicalContentLibraryPremium.
...
const getTreatmentSuggestion = (key: string): TreatmentSuggestion => {
  const content = clinicalContentLibraryPremium[key];
  if (!content) {
    console.warn(`Content for treatment key "${key}" not found in clinicalContentLibraryPremium.`);
    return {
...
/**
 * Sugiere tratamientos para el MÓDULO PREMIUM basado en el EvaluationState completo.
...
export function suggestTreatmentsPremium(evaluation: EvaluationState): TreatmentSuggestion[] {
