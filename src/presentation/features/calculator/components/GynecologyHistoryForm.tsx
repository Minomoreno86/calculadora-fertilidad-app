...
export const GynecologyHistoryForm = <TFormValues extends FieldValues>({ control }: Props<TFormValues>) => {
  ...
  return (
    <View style={styles.container}>
      ...
      <ControlledTextInput control={control} name={"infertilityDuration" as Path<TFormValues>} label="Duración de infertilidad (años)" keyboardType="number-pad" placeholder="Ej: 2" />
      <ControlledTextInput control={control} name={"cycleLength" as Path<TFormValues>} label="Duración promedio del ciclo (días)" keyboardType="number-pad" placeholder="Ej: 28" />

      <ControlledOptionSelector control={control} name={"endometriosisStage" as Path<TFormValues>} label="Endometriosis" options={options.endometriosis} />
      <ControlledOptionSelector control={control} name={"myomaType" as Path<TFormValues>} label="Miomas Uterinos" options={options.myoma} />
      <ControlledOptionSelector control={control} name={"polypType" as Path<TFormValues>} label="Pólipos Endometriales" options={options.polyp} />
      <ControlledOptionSelector control={control} name={"adenomyosisType" as Path<TFormValues>} label="Adenomiosis" options={options.adenomyosis} />
      <ControlledOptionSelector control={control} name={"hsgResult" as Path<TFormValues>} label="Resultado Histerosalpingografía (HSG)" options={options.hsg} />

      <ControlledSwitch control={control} name={"hasPcos" as Path<TFormValues>} label="¿Tiene SOP diagnosticado?" />
      <ControlledSwitch control={control} name={"hasPelvicSurgery" as Path<TFormValues>} label="¿Cirugías pélvicas previas?" />
      {control._formValues.hasPelvicSurgery && (
        <ControlledTextInput control={control} name={"numberOfPelvicSurgeries" as Path<TFormValues>} label="Número de cirugías pélvicas" keyboardType="number-pad" placeholder="Ej: 1" />
      )}
