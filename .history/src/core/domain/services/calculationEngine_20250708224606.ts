...
export function calculateProbability(userInput: UserInput): EvaluationState {
  ...
  evaluation = {
    ...
    ...endometriosisResult, ...myomasResult, ...adenomiosisResult, ...polypsResult,
    ...hsgResult, ...otbResult, ...amhResult, ...prolactinResult, ...tshResult,
    ...homaResult, ...maleFactorResult,
    ...infertilityResult, ...pelvicSurgeryResult,
    // comentario_adenomiosis: adenomiosisResult.comentario ?? '', // Removed because 'comentario' does not exist
  };
  ...
  const factorsToMultiply = [
    ...
    evaluation.endometriosis_factor, evaluation.mioma_factor,
    adenomiosisResult.spontaneousFactor ?? 1.0,
  ...
  if (isNaN(evaluation.pronostico_numerico)) {
    evaluation.pronostico_numerico = 0;
    evaluation.pronostico_categoria = "ERROR";
    evaluation.pronostico_frase = "No se pudo calcular la probabilidad.";
    evaluation.benchmark_frase = "";
    evaluation.recomendaciones_lista.push("Verifique que todos los datos estén completos y correctos.");
  }
