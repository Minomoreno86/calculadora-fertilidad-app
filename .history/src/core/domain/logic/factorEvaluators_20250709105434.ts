...
export const evaluateAgeBaseline = (age: number): PartialEvaluation => {
  if (age <= 24) return { factors: { baseAgeProbability: 27.5 }, diagnostics: { agePotential: "Fertilidad excelente" } };
  if (age <= 29) return { factors: { baseAgeProbability: 22.5 }, diagnostics: { agePotential: "Fertilidad muy buena" } };
  if (age <= 34) return { factors: { baseAgeProbability: 17.5 }, diagnostics: { agePotential: "Buena fertilidad" } };
  if (age <= 37) return { factors: { baseAgeProbability: 12.5 }, diagnostics: { agePotential: "Fecundidad en descenso" } };
  if (age <= 40) return { factors: { baseAgeProbability: 7.5 }, diagnostics: { agePotential: "Reducción significativa" } };
  if (age <= 42) return { factors: { baseAgeProbability: 4.0 }, diagnostics: { agePotential: "Baja tasa de embarazo" } };
  if (age >= 43) return { factors: { baseAgeProbability: 1.5 }, diagnostics: { agePotential: "Probabilidad casi nula" } };
...
export const evaluatePcos = (hasPcos: boolean, bmi: number | null, cycleDuration?: number): PartialEvaluation => {
  ...
  if (bmi && bmi >= 25) { factor *= 0.9; severity = "Moderado"; }
  if (cycleDuration && cycleDuration > 35) { factor *= 0.85; severity = "Moderado"; }
  if ((bmi && bmi >= 30) || (cycleDuration && cycleDuration > 45)) { severity = "Severo"; }
...
export const evaluateEndometriosis = (grade: number): PartialEvaluation => {
  if (grade >= 1 && grade <= 2) return { factors: { endometriosis: 0.85 }, diagnostics: { endometriosisComment: "Endometriosis leve (Grados I-II)" } };
  if (grade >= 3 && grade <= 4) return { factors: { endometriosis: 0.60 }, diagnostics: { endometriosisComment: "Endometriosis severa (Grados III-IV)" } };
  return { factors: { endometriosis: 1.0 } };
};

export const evaluateMyomas = (type: MyomaType): PartialEvaluation => {
  if (type === 'submucosal') return { factors: { myoma: 0.30 }, diagnostics: { myomaComment: "Mioma submucoso detectado" } };
  if (type === 'intramural_large') return { factors: { myoma: 0.60 }, diagnostics: { myomaComment: "Mioma intramural grande detectado" } };
  return { factors: { myoma: 1.0 } };
};

export const evaluateAdenomyosis = (type: AdenomyosisType): PartialEvaluation => {
    if (type === 'focal') return { factors: { adenomyosis: 0.8 }, diagnostics: { adenomyosisComment: "Adenomiosis focal" } };
    if (type === 'diffuse') return { factors: { adenomyosis: 0.5 }, diagnostics: { adenomyosisComment: "Adenomiosis difusa" } };
...
export const evaluateHoma = (homaValue?: number): PartialEvaluation => {
  ...
  if (homaValue >= 2.5 && homaValue < 4.0) return { factors: { homa: 0.95 }, diagnostics: { homaComment: "Resistencia a la insulina leve" } };
  if (homaValue >= 4.0) return { factors: { homa: 0.90 }, diagnostics: { homaComment: "Resistencia a la insulina significativa" } };
...
export const evaluateMaleFactor = (input: UserInput): PartialEvaluation => {
  ...
  if (alterations.length === 0) return { factors: { male: 1.0 }, diagnostics: { maleFactorDetailed: "Parámetros seminales normales" } };
